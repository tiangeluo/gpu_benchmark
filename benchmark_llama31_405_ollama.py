...
#model_path = "/data/tiangel/Meta-Llama-3.1-405B-Instruct-AWQ-INT4"
#
## AWQ Configuration
...
def process_batches(prompts, batch_size):
    ...
    while start_index < len(prompts):
        ...
        #response = ollama.generate(model='llama3.1:405b-instruct-q3_K_M', prompt=batch_prompts[0])
        #response = ollama.generate(model='llama3.1:70b-text-q4_K_M', prompt=batch_prompts[0])
        response = ollama.generate(model='llama3.1:405b-text-q4_K_M', prompt=batch_prompts[0])
        #response = ollama.generate(model='mistral-large', prompt=batch_prompts[0])
        #response = ollama.generate(model='mistral-large:123b-instruct-2407-q8_0', prompt=batch_prompts[0])
        ...
        print(start_index, len(prompts), np.mean(token_s))
        start_index += batch_size
        token_s.append(response['eval_count']/response['eval_duration']*(10**9))
    return results, token_s

results, token_s = process_batches(prompts, max_batch_size)

end_time = time.time()
elapsed_time = end_time - start_time

print('token/s', np.mean(token_s))
